...
BYTE* shiftRows(BYTE *block, int mode){ 
    ...
    memcpy(block, buffer, sizeof(BYTE)*BLOCK_SIZE);
    return block;
}
...
void AES128(BYTE *input, BYTE *result, BYTE *key, int mode){
	...
	for(int j = 0; j < 9; j++)
	{
		...
		for(int k = 0; k < 16; k++)
		{
			input[j*(64/BLOCK_SIZE) + k] = block[k];
		}
	}
	...
	memcpy(result, block, sizeof(BLOCK_SIZE));	
}
